...
namespace Parapharmacie
{
    public partial class ventes : Form
    {
        ...
        {
            Con.Open();
            SqlCommand cmd = new SqlCommand("select Id from clients", Con);
            SqlDataReader Rdr;
        ...
        {
            Con.Open();
            string Req = "select * from Ventess";
            SqlDataAdapter sda = new SqlDataAdapter(Req, Con);
            SqlCommandBuilder builder = new SqlCommandBuilder(sda);
            var ds = new DataSet();
            sda.Fill(ds);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                if(Convert.ToInt32(qttb.Text) > Convert.ToInt32(stocktb.Text) || Convert.ToInt32(stocktb.Text)==0)
                {
                    MessageBox.Show("stock indisponible");
                }
                else
                {
                    try
                    {
                        ...
                        string Req = "insert into Ventess values('" + comboBox2.SelectedValue.ToString() + "','" + dateTimePicker1.Value.Date + "','" + prodtb.Text + "','" + prixtb.Text + "','" + qttb.Text + "','" + retb.Text + "','" + montant + "','" + desctb.Text + "')";
                        SqlCommand cmd = new SqlCommand(Req, Con);
                        cmd.ExecuteNonQuery();

                        MessageBox.Show("vente ajoute");
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    Con.Open();
                    string Req = "delete from Ventess where Id=" + Cle + "";
                    SqlCommand cmd = new SqlCommand(Req, Con);
                    cmd.ExecuteNonQuery();
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    Con.Open();
                    int montant = Convert.ToInt32(qttb.Text) * Convert.ToInt32(prixtb.Text) - Convert.ToInt32(retb.Text);
                    string Req = "Update Ventess set client='" + comboBox2.SelectedValue.ToString() + "',date_de_vente='" + dateTimePicker1.Value.Date + "',produit='" + prodtb.Text + "',prix=" + prixtb.Text + ",quantite=" + qttb.Text + ",remise="+retb.Text+ ",prix_total='" + montant + "',description='" + desctb.Text + "' where Id=" + Cle + ";";
                    SqlCommand cmd = new SqlCommand(Req, Con);
                    cmd.ExecuteNonQuery();

                    MessageBox.Show("vente modifié");
        ...
        {
           /* printDocument1.DefaultPageSettings.PaperSize = new System.Drawing.Printing.PaperSize("pprnm", 258, 600);
